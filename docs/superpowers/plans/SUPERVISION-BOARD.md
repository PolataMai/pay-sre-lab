...
| R1 切片一收尾 | ✅ 已验收 | |
| R4 策略目录化 | ✅ 已验收 | |
| R5a 渠道拒绝尖峰 | ✅ 已验收 | `0016d73` 隔离回归绿 |
| R5b 返回码配置错误 | ✅ 已验收 | C2 修复 `295525b` 验收通过 |
| R5c 回调丢失 | ⚠️ 代码已提交，欠 spec 审查（C5） | `151290a` |
| R5d 多渠道路由 | ⚠️ 代码已提交，欠 spec 审查（C5） | `48e0872` |
| R6/R7 控制台与注入 UI | ⚠️ 代码已提交，未验收（前端未过质检） | `9d5e0b8`，compose 增 pay-sre-console |
| R8 | ❌ 未开始 | 仅有目录报告工具（`551be0c`，非评分矩阵） |
| R9 | ⚠️ 工具已提交（带 spec） | `23f09ad` |
| R2 CI 验收 | ⏸ 等外部条件 | 需在 PR #1 观察 Actions |
| R3 凭据联调 | ⏸ 等用户提供 | Nacos 地址 + MiniMax key |

**终局 HEAD `9d5e0b8` 隔离全量回归：BUILD SUCCESS，228 测试全绿（本地排除 Docker 拉镜像用例）。**
监工修复记录：C3 部署产物破坏已由监工代修（`b287da5`）并加守护断言。
