...
- Loki 3 response normalization accepts both flat metadata and categorized `structuredMetadata`/`parsed` envelopes. Structured metadata has precedence so query-time parsed labels cannot replace source trace or payment fields.
