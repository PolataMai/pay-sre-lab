...
- Loki reads explicitly request `X-Loki-Response-Encoding-Flags: categorize-labels`, because the default query response omits structured metadata. The Collector preserves the original ECS JSON line and read queries use `| json` before allowlisted payment/event filters, avoiding version-specific OTLP attribute key mapping. Normalization also accepts the legacy flat form; source structured metadata has precedence so query-time parsed labels cannot replace source trace or span fields.
