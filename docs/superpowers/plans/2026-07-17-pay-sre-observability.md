...
- Loki reads explicitly request `X-Loki-Response-Encoding-Flags: categorize-labels`, because the default query response omits structured metadata. Normalization also accepts the legacy flat form; source structured metadata has precedence so query-time parsed labels cannot replace source trace or payment fields.
